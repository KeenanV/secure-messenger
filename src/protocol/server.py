...
class Server:
    ...
    def run(self): 
        while True:
            ...
            #     str = os.urandom(16)
            #     self.pm.queue(data=["connection init", user[1], cid, str, self.users[user[1]]["addr"], self.users[user[1]]]["pub_key"]],
            #  flag=None, uid=user[0])
            #     self.pm.queue(data=["connection init", user[0] cid, str, self.users[user[0]]["addr"], self.users[user[0]]]["pub_key"]], 
